...
$id = $_GET['service_Id'];
   if (isset($_POST['delete'])){
        ...
        $users=mysqli_query($con ,"DELETE FROM article WHERE count_id='$id'");
...
if ($users) {
   ...
   echo "<div class='alert alert-success tex-center' style='width: 30%; margin-left: 34%;margin-top: 5%;text-align:center;'>Article Deleted Successfully.<img src='../icons/action_check.gif' alt='image' class='photo' style='width: 20px; height: 20px;border-radius: 300px; margin-top: 1px;' style='align-text:center;'><br> <a href='addarticle.php'>Home</a></div>
    ...
    echo "Error: ".$users."<br />". $con->error;
            ...
            $services = "select * from article where count_id='$id'";
            $results = $con->query("$services");
            $product = $results->fetch_assoc();
            $service_tag= $product['title'];
            ...
            <form method='post'>
                ...
                <h4 class='text-center'>Confirm Article Deletion!</h4>
                <img src='../icons/icon_error1.gif' alt='image' class='photo' style='width: 30px; height: 30px;border-radius: ; margin-top: 3px; '><br>
                    You have requested to delete <br>article: <b>$service_tag</b>,<br> material_Id.  :<tt class='font-italic font-weight-bold'>$service_Id</tt>.<br> Are you sure?
