...
if($balance<=0){
  $status='complete';
  if($role=='SuperAdmin' or $role=='accountant'){
 ...
 //$order4=mysqli_query($con ,"DELETE FROM myorder WHERE  status='complete'");
    ...
    <a href='transaction.php?service_Id=$service_Id&price=$price' style='text-decoration:none;'>Print Receipt </a>
            ...
            <form method='post'>
                ...
                <div class='alert alert-warning tex-center' style='background:;width:30%;position:center;margin-left:35%;padding-bottom:3px; border: ; text-align:center; >
                <h4 class='text-center' style='color:red; text-align:center;'>Confirm Transaction commit!</h4>
                
                  <div class='alert alert-warning tex-center' style='text-align:center;'> <img src='../icons/icon_error1.gif' alt='image' class='photo' style='width: 30px; height: 30px;border-radius: ; margin-top: 3px;  '><br>You are about to complete:  <br>Order: <b>$service_tag</b>,<br> Order Id.  :<tt class='font-italic font-weight-bold'>$service_Id</tt>.<br> Are you sure?
