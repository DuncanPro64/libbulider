...
$id = $_GET['service_Id'];
   if (isset($_POST['delete'])){
        ...
        $users=mysqli_query($con ,"DELETE FROM material WHERE rawmat_id='$id'");

if ($users) {
   
   echo "<div class='alert alert-success tex-center' style='width: 30%; margin-left: 34%;margin-top: 5%;text-align:center;'>material Deleted Successfully.<img src='../icons/action_check.gif' alt='image' class='photo' style='width: 20px; height: 20px;border-radius: 300px; margin-top: 1px;' style='align-text:center;'><br> <a href='addmaterial.php'>Home</a></div>
               ";
}
else{
    echo "Error: ".$users."<br />". $con->error;
            ...
            $services = "select * from material where rawmat_id='$id'";
            $results = $con->query("$services");
            $product = $results->fetch_assoc();
            $service_tag= $product['material_name'];
            $service_Id = $product['rawmat_id'];
