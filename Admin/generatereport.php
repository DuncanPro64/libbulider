...
require_once('../User/Dbconnection.php');
...
require_once('../tcpdf/tcpdf.php');
    ...
    $id=$_GET['service_Id'];
    $update=false;
   $result=mysqli_query($con,"SELECT c.first_name, c.last_name,c.email_address, c.phone_number ,c.rawcust_id,o.transaction_number,o.rawcust_id, o.transacted_at,o.total_price,o.quantity FROM customer AS c
 INNER JOIN transaction AS o
 ON c.email_address= o.rawcust_id
 WHERE o.rawcust_id='$id'
 ORDER BY c.email_address, o.transaction_number; ");
    $code = rand(1000, 9999);
        ...
        while($row = mysqli_fetch_array($result))
            {
                ...
                $phone=$row['phone_number'];
                 ...
                 $rawcust_id=$row['rawcust_id'];
                  $DOR=$row['transacted_at'];
                   $Ref=$row['transaction_number'];

            }
          }
           $result2=mysqli_query($con,"SELECT c.material_name,c.rawmat_id,o.rawcust_id, o.rawtra_id , o.transaction_number, o.transacted_at, o.total_price,o.transaction_id,o.quantity FROM material AS c
 INNER JOIN transaction AS o
        ...
        while($row = mysqli_fetch_array($result2))
            {
                ...
                $order_number=$row['transaction_number'];
                $order_date=$row['transacted_at'];
                 $total_price=$row['total_price'];
                 $material_name=$row['material_name'];
                  $rawmat_id=$row['rawmat_id'];
                  $email_address=$row['rawcust_id'];
                   $raword_id=$row['rawtra_id'];
...
class PDF extends TCPDF {
  ...
  //initilalize variables
           $result3=mysqli_query($con,"SELECT c.material_name, o.rawmat_id,o.rawcust_id, o.rawtra_id , o.transaction_number, o.transacted_at, o.total_price,o.transaction_id FROM material AS c
 INNER JOIN transaction AS o
 ON c.rawmat_id = o.rawmat_id
WHERE o.rawcust_id='$id'
 ORDER BY c.material_name, o.transaction_number; ");
 ...
 $select= 'HiQ inc';
        ...
        $this->Cell(51,5,'Powered By-'.$select ,0,1);
        $this->Cell(51,5,'0714757251' ,0,1);
...
$rate=2/100;
  $i=1;
  $result4=mysqli_query($con,"SELECT c.material_name,c.unit_price, o.rawmat_id,rawcust, o.rawtra_id , o.transaction_number,o.quantity, o.transacted_at, o.total_price,o.transaction_id FROM material AS c
 INNER JOIN transaction AS o
 ON c.rawmat_id = o.rawmat_id
 WHERE o.rawtra_id=$id
...
$pdf->writeHTML($html);
$html = '<table data-page-length="5" width="655px" height="655px" border="none"  style="text-align:justify;text-align:center; font-size: 11px;font-family:Times; border-bottom:1px solid black;">';
...
$total_price="";
$email_address="";
$result5=mysqli_query($con,"SELECT c.material_name,c.unit_price, o.rawmat_id,o.rawcust_id, o.rawtra_id ,o.transaction_id,o.quantity, o.transaction_number, o.transacted_at, o.total_price,o.transaction_id FROM material AS c
 INNER JOIN transaction AS o
 ON c.rawmat_id = o.rawmat_id
WHERE o.rawcust_id='$id'
 ORDER BY c.material_name, o.transaction_number; ");
